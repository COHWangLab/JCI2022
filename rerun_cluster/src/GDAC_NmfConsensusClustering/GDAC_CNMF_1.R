...
main <- function(...) {
	...
	outputprefix
	consensusNMF(input.ds = norfile[1], k.init = k.int, k.final = k.final, num.clusterings = 20, maxniter = 1500, error.function = "divergence", rseed = 123, stopconv = 40, stopfreq = 10, non.interactive.run = F, doc.string = outputprefix, libdir = libdir)
}
...
consensusNMF <- function(input.ds, k.init, k.final, num.clusterings, maxniter, error.function, rseed=123456789,stopconv = 40, stopfreq = 10, non.interactive.run = F, doc.string = "", libdir = "", ...) {
	...
	for (k in k.init:k.final) {
		...
		assign <- foreach(i = 1:num.clusterings, .combine = cbind) %dopar% {
			# Find the nonnegative matrix factorization
		   	...
		   	save(NMF.out,file=paste(directory, doc.string, ".", k,"_NMF.out.Rdata", sep=""))
			# Find the membership
			members <- c()
			#jit(1)
			for (j in 1:cols) { 
	...
	CairoPDF(file =  paste(directory, doc.string, ".", "consensus.all.k.plot.pdf", sep=""))
	...
	for (k in 1:num.k) { 
		CairoPDF(file = paste(directory, doc.string, ".", "consensus.plot.k", k.vector[k], ".pdf", sep=""), width = 6, height = 6)
	...
	CairoPDF(file = paste(directory, doc.string, ".cophenetic.coefficient.pdf", sep = ""), width = 6, height = 8)
	y.range <- c(1 - 2*(1 - min(rho)), 1)
	plot(k.vector, rho, main ="Cophenetic Coefficient", xlim=c(k.init, k.final), ylim=y.range, xlab = "k", ylab="Cophenetic correlation", type = "n")
	lines(k.vector, rho, type = "l", col = "black")
	points(k.vector, rho, pch=22, type = "p", cex = 1.25, bg = "black", col = "black")
